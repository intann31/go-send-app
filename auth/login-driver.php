...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!empty($email) && !empty($password)) {
        // Lakukan JOIN ke tabel user untuk mengambil password dan nama_lengkap
        $query = mysqli_prepare($conn, "
            ...
            WHERE user.email = ? OR driver.email = ?
        ");
        
        mysqli_stmt_bind_param($query, "ss", $email, $email);
        ...
        if ($driver = mysqli_fetch_assoc($result)) {
            if ($password === $driver['password'] || password_verify($password, $driver['password'])) {
                ...
                $_SESSION['nama_driver'] = $driver['nama_lengkap'];
...
<div class="register-text">
    Ingin menjadi mitra? <a href="register-driver.php">Daftar Sekarang</a>
</div>
